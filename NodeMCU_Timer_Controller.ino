...
void setup() {
  ...
  checkForUpdates();
  server.send(200, "text/plain", "Update check initiated. See serial monitor for status.");
});
}
...
void loop() {
  ...
  // Check for Git Hub updates every hour
...
void checkForUpdates() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(client, githubRawURL);
    int httpCode = http.GET();
    
    if (httpCode == 200) {
      String payload = http.getString();
      
      // Look for version in the code
      int versionIndex = payload.indexOf("const String currentVersion = \"");
      if (versionIndex != -1) {
        int startIndex = versionIndex + 31; // Length of search string
        int endIndex = payload.indexOf("\"", startIndex);
        String remoteVersion = payload.substring(startIndex, endIndex);
        
        if (remoteVersion != currentVersion) {
          Serial.println("Update available: " + remoteVersion);
          performUpdate();
        } else {
          Serial.println("Already up to date: " + currentVersion);
        }
      }
    }
    http.end();
...
void performUpdate() {
  ...
  t_httpUpdate_return ret = ESPhttpUpdate.update(client, githubRawURL);
